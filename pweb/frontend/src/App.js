...
const App = () => {
  ...
  const addTodo = () => {
    if (!task.trim() || !dueDate) return;  // Ensure both task and dueDate are provided

    axios.post('http://localhost:5000/todos', { task, dueDate })
      .then(response => {
        setTodos(prevTodos => {
          const updatedTodos = [...prevTodos, response.data];
          return updatedTodos.sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)); // Sort after adding
        });
      })
